...
class DeviceFileInserter extends AbstractFileInserter implements DatabaseUpdateInterface {
   ...
   private \Set<int> $lasik_mdr_report_keys;

   public function __construct(string $file_name, \PDO $pdo_maude_recent)
   {
      parent::__construct($file_name, $pdo_maude_recent);

      $this->LogFile = new SplFileObjectExt( "device-file-inserter.log" ," w"); 
      $this->new_lasik_mdr_report_keys = new Set<int> {}; // <-- Expected class name
      /*
    ...
    protected function processLine(string $text, int $line_number) : void
    {
       $matches = array();
       ...
       try {
        ...
        if (!isset($matches[1]) ) {
         ...
         /* prior code
         if (binary_search($this->mdr_report_key, $this->prior_mdr_report_keys)) { // if already in table, ignore.
