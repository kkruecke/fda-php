 ...
 */
function create_date(string $input)
{
...
class MdrFileInserter extends AbstractFileInserter implements DatabaseUpdateInterface { 
   ...
   private $LogFile;    // SplFileObjectExt 
   private $foidev_mdr_report_keys; // \Ds\Set ?
   private $max_of_foidev_mdr_report_keys;

   private $insert_stmt; // PDOStatement
   private $regex;   // string
   private $insert_count; // int
   private $mdr_report_key; // int
   private $report_source_code; // string
   private $date_received;  // string

  public function __construct(string $file_name, \PDO $pdo_handle)
  {
  ...
  public function processLine(string $text, int $line_number) : void
  {
      $matches = array();
      ...
      } else if (binary_search($this->mdr_report_key, $this->foidev_mdr_report_keys) === FALSE) {
  ...
  protected function create_date(string $input) : string
  {
     $date_parts = explode("/", $input); // date is in format of MM/DD/YYYY      
